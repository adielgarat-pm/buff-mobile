...
- [ ] **Send from `adi@buffadhd.com`** — Adi's domain email via Google Workspace. לא noreply@. Founder voice = founder address.
...
- **From:** `adi@buffadhd.com` (Adi's domain email via Google Workspace).
...
1. **Today (60 min):** Read this kit. Adjust offer spec if needed. Use `adi@buffadhd.com` (Adi's domain email via Google Workspace).
