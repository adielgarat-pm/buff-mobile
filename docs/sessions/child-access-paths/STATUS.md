...
- **Gender branch** verified for BOTH genders at runtime. Girl (EN): `card3Sub_f` rendered, no raw-key artifact (EN `_m`==`_f`, genderless). Boy (**HE run**, child ZTestBoy): ChildAccessStep shows the masculine set — `card1Title_m` = **"בטלפון שלו"** (his phone), `card3Sub_m` = **"הרגע שלו על המכשיר שלך — הוא מסמן בעצמו"** — i.e. שלו/הוא-מסמן, the masculine forms vs the feminine שלה/היא-מסמנת. `genderSuffix('בן'→_m)` confirmed. Native ordering + "אשלח הערב" (send-tonight) secondary hold in Hebrew too.
- **shared_device tap → `access_mode='shared_device'` WRITTEN to profile** (confirmed in JS logs; passes the CHECK constraint) → UStep8_Complete → "Let's start with {child}" / HE **"בואו נתחיל עם {child}"** CTA → **View-as-Child landing** (child welcome → Vibe Check → 3 auto-generated tasks loaded via useChildData). **No login screen. No stuck modal.** (Verified in both EN and HE runs.)
- **Dashboard "🌱 {child}'s moment" / "🌱 הרגע של {child}" button RENDERS** on the shared_device card (Chunk 3b; `child.accessMode === 'shared_device'` gate), and **re-entry works**: tapping it enters View-as-Child (verified on ZTestBoy).

Harness finding: dashboard child-card buttons (moment, "View as Child", "+ Bonus") and even the bottom tab bar ignore `adb input tap` (fast down-up missed by RN's touch responder). **`adb shell input motionevent DOWN x y` + `… UP x y` with a ~150ms gap works** — that's how the moment re-entry (and the whole HE Settings→language flow) was driven. Worth adding to the buff-testing helpers.
