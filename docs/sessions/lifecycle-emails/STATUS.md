...
| Phase 2 — OAuth consent ask (client) | ✅ code complete | 2026-07-17 | (this PR) | tsc clean; i18n guard tests PASS (5/5); web bundle compiles. Runtime UI check on Android/Web = Adi (auth-gated: needs a Google-signup parent) | `useMarketingConsentPrompt` in ParentTabs; crossAlert (both platforms); asked-once stamped in onboarding_data; never defaults to true. Ships via next build/OTA (CI publish only per IN OTA-fingerprint learning) |
