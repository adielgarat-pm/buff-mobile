...
**Slug:** `activities-multi-day` · **Branch (proposed):** `pkg/activities-multi-day`
...
1. **Approve the SPEC amendment** (promote multi-weekday from the parent SPEC's v2 to now) + **schema change** (`weekdays` column + backfill). Both need your explicit yes.
2. **All-7 label:** collapse to "כל יום / Every day", or just list all seven? (Recommend collapse — cleaner.)
3. **Duplicate button:** drop entirely (my recommendation, since multi-day supersedes it), or still want it as a separate convenience? (Recommend drop.)
