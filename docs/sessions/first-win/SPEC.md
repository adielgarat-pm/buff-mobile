...
> **Status:** v1 DRAFT — **superseded in part by `REVIEW.md` (2026-09-24 adversarial review)**. §1 metric, §3 ranking and §7 Values Check are replaced by REVIEW.md §3–§5 and TESTS.md §A. Adi decided D1–D3 on 2026-09-24 (REVIEW.md §3). Awaiting `approved, proceed` for P0. No code has been written.
...
| **C3** | **Winnable first mission + visible payoff.** While a child has 0 real completions, the child dashboard (mint + gamer) pins one "First mission" card on the easiest starter task, plus a single line of progress toward their cheapest reward ("{n} BUFFs to {reward}"). The card disappears after the first win. | ~~High (68% open but never complete)~~ Low reach: most children never log in (REVIEW F1); deferred | M | none | copy; which task counts as "easiest" (below) |
