...
| **11: Spec sync + PR** | 🟡 _pending_ | — | — | Canonical doc updates (PRD §9.2, AUDIT S-01, FEATURE_PRIORITIZATION F-039+F-063, GAP_ANALYSIS); INTEGRATION_LEARNINGS entries for the GRANT root cause; STATUS closeout; PR opened against main. Awaiting Adi device-test (install dev build → grant notif permission → fire `parent_sos` → verify push lands on phone). | — |
