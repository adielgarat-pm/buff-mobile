...
| **3: Edge Function dispatch** | 🟡 _source-ready, awaiting deploy_ | 2026-05-20 | (this commit) | Firebase project `buff-mobile-prod` created by Adi 2026-05-20 18:01. Service account JSON received. Edge Function source written: `supabase/functions/push-notification-fanout/index.ts` (handles all 6 v1 push types, activity suppression, FCM HTTP v1 OAuth2 + dispatch, dead-token cleanup, idempotency via notification_pushes PK). Deploy via MCP blocked temporarily by classifier — will retry. **Adi action items**: (1) set `FCM_SERVICE_ACCOUNT_JSON` secret in Supabase Dashboard → Edge Functions → Secrets; (2) configure Database Webhook on `notifications` INSERT → Edge Function URL. | — |
