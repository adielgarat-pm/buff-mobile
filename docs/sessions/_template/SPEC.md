...
> מה כל אחד מהצדדים יכול ולא יכול לעשות בחבילה הזו. **Claude.ai ממלאה את זה בתחילת החבילה.**
...
**Values Check Pass:** [ ] כן / [ ] לא — אם לא, לא ממשיכים.

---

## AI Token Cost (חובה אם החבילה כוללת קריאת LLM כלשהי)

> D-2026-07-17 (lifetime מול עלויות טוקנים): כל פיצ'ר AI חדש מקבל הערכת עלות בשלב ה-SPEC.
> **מתחת ל~₪1/חודש/משפחה → נכלל ב-lifetime/premium עם מכסה (cap). מעל → נולד עם מודל משלו**
> (קרדיטים / תוסף בתשלום) — lifetime לעולם לא מבטיח רטרואקטיבית פיצ'רים עתירי-עלות.

- **מודל:** (למשל gemini-2.5-flash)
- **טוקנים לקריאה (in/out, הערכה):**
- **מכסה (cap):** (למשל 3/ילד/שבוע — חובה שיהיה cap)
- **עלות חודשית למשפחה בתקרת המכסה:** ₪
- **מסקנה:** [ ] נכלל ב-premium/lifetime / [ ] מודל תמחור נפרד
...
> אילו prompts (DB rows + /docs/PROMPTS.md אם יש) משתנים, ומה ההתנהגות החדשה. כולל rollback strategy.
...
> מסכים, copy, components. הפניה ל-teen-ui-design/ אם רלוונטי.
