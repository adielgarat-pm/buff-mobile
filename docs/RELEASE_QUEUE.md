...
| 2026-06-16 | #251 / `pkg/expo-web-app` | feat | **App runs as an installable web app (Expo Web) for iPhone users.** Native-only modules guarded/`.web`; web Google OAuth (`detectSessionInUrl` on web) — fixes empty-dashboard-after-login; skip RevenueCat on web; hide native-only timetable import on web; **PWA** (manifest + apple-* meta + service worker → "Add to Home Screen"). **Every change is `Platform.OS==='web'`-gated → zero Android behavior change.** Ships to Vercel (`app.buffadhd.com`), NOT via the Android train — deploy pending Adi's provider config. | Train | no (web-only; no Android impact) | (web build only) |
| 2026-06-16 | #253 / `pkg/kid-delight-parity` | feat | **Completion sound + confetti (sensory parity).** Gentle success chime (`expo-audio`, self-generated WAVs in `assets/sfx`) + a brief confetti burst on every real task completion — both Mint + Gamer, all screens (emitted from `useChildProgress.completeTask`); mute toggle in the child Menu. Closes the sensory gap that drove a real kid back to the old Lovable app. Also carries a **realtime fix** (unique channel names in `useAppSettings`/`useChildrenDashboard` → fixes a dev/web `.on() after subscribe()` white-screen; no Android behavior change). New dep: `expo-audio` (approved). tsc clean; jest; web bundles+loads clean. Sound/confetti *feel* + native Hat-3 pending Adi. | Train | yes | Child Quests/HQ → complete a task → chime + confetti; child Menu → 🔊 Sounds toggle |
