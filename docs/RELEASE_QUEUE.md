...
| 2026-06-08 | `fix/hide-dev-subscribe-toggle` (PR #197) | fix | "🛠 Dev: Simulate Subscribed" toggle in parent Settings is now wrapped in `__DEV__` — only appears in dev builds, never to testers/users on release builds (it was rendering unconditionally). No effect for users with a real lifetime/subscription flag; also unblocks clean marketing screenshots | Train | yes | Parent Settings render — no dev toggle on a release build |
