...
## 24 בספטמבר 2026 — Freemium v2: הכל חינם חוץ מה-AI

### D-2026-09-24-01: Freemium v2 — ליבה חינם לכל המשפחה, BUFF Coach (AI) בתשלום עם ניסיון של 14 יום (מחליף חלקים מ-D-2026-06-19-01 ומ-D-2026-07-04-01)

**ההחלטה (אושרה ע"י Adi, 2026-09-24):**
- **חינם לתמיד:** כל לולאת ההרגל וכל מה שהילד נוגע בו — משימות וילדים בלי הגבלה, BUDDY וסקינים, BUFFs, חנות ומימוש, Vibe Check / SOS / Pause / Anchor, התראות, מערכת שעות בהזנה ידנית, פעילויות, הכנת תיק, יציאה משגרה. **הנתונים הסטטיסטיים במסך התובנות** (מה עובד, מפת השבוע, ציר המשימות) חינם.
- **בתשלום — BUFF Coach ($9.99 לחודש / $59.99 לשנה):** אזורי ה-AI בלבד: מאמן ותובנות AI, קליטה חכמה (תמונה/טקסט → משימות), **ייבוא מערכת שעות מתמונה**.
- **ייבוא מערכת שעות:** משפחה בלי זכאות מקבלת **ייבוא אחד בחינם** (היום הראשון של הייבוא, כולל ניסיונות חוזרים). אחר כך — BUFF Coach. הזנה ידנית תמיד חינם. (מחליף את ההחלטה מ-2026-07-29 "בלי שער לייבוא".)
- **ניסיון:** 14 יום של BUFF Coach מרגע שהילד משלים **משימה אמיתית ראשונה** (לא ההרשמה, לא משימת הדוגמה באונבורדינג). אחרי הניסיון: תובנת AI אחת בחינם בשבוע.
...
- **שיווק:** "חינם לכל המשפחה. מאמן AI לבחירה." — לא "100% חינם", ולא "ניסיון חינם" בפרסומות בלי תנאים.

**הסיבה:** צוואר הבקבוק הוא אקטיבציה, לא המרה (68% מהילדים לא משלימים משימה ראשונה; 0 משפחות משלמות). פייוול לפני הניצחון הראשון רק מאבד משתמשים. ה-AI כמעט בלתי נראה (משפחה 1 מ-51), והניסיון חושף אותו לכל משפחה שמתחילה לעבוד. מקור: `docs/research/FREEMIUM_STRATEGY_2026-09.md`, `docs/research/VALUE_VALIDATION_2026-09.md` §8.

**בחינה מחדש מוסכמת:**
- כשיהיו **לפחות 100 משתמשים פעילים** — לבחון שוב אם לייצר תשלום גם בלי ה-AI, כולל האם הנתונים הסטטיסטיים במסך התובנות עוברים לפרימיום.
- אם 60 יום אחרי ההשקה פחות מ-1% מהמשפחות שסיימו ניסיון משלמות — להוסיף מנוף סקייל שרק משפחות מעורבות פוגשות, במקום להקדים חיכוך.

**תנאי ערכים:** הודעת סיום הניסיון לא ממסגרת אובדן; שום מחיר או פייוול לא מוצגים לילד.

**הערה:** טיוטת `docs/research/DRAFT_DECISION_paywall_H8.md` (Option A) בוטלה — Freemium v2 מחליף אותה.

**מסמכים מושפעים:** `docs/BUFF_PRD.md`, `docs/sessions/freemium-v2/`, `docs/BUFF_FAQ.md`, `docs/BUFF_MESSAGING.md`, `docs/marketing-assets/play-store-listing/README.md`, `landing-web/src/pages/Pricing.tsx`; קוד: `useSubscription.ts`, `PaywallScreen.tsx`, `ParentInsightsScreen.tsx`, `TimetableScreen.tsx`, `supabase/functions/parse-schedule`, migration 058.

---

## 4 באוגוסט 2026 — Web מוכר כמסלול הפעלת-ילד

### D-2026-08-04-01: web = מסלול הפעלת-ילד לגיטימי (מחליף את DG1 של onboarding-redesign)

**ההחלטה:** ה-web מוכר כמסלול שבו **ילד מפעיל את BUFF באופן קבוע**, לא רק surface להתקנה/מסירה. זה **מחליף** את ההחלטה מ-`onboarding-redesign` DG1 (יולי 2026: "web = surface להתקנה/מסירה, לא הפעלת-ילד"). בעקבות זה, פקג' `child-access-paths` מציע "מחשב/טאבלט בבית" ככרטיס גישה שווה-מעמד.

**הסיבה:**
- ההחלטה מיולי נשענה על 0/9 משפחות web פעילות — קוהורט קטן (רווח סמך על 0/9 מגיע עד ~30%).
- ראיה הפוכה מ-2026-08-04: family `de60` — ילד web עם לוגין משלו, 11 ימים פעילים, 29 השלמות, נראה 2026-08-04. זו הוכחת-קיום שילד כן מקיים לולאה יומית ב-web.
- כלל CLAUDE.md: החלטה סגורה לא נהפכת בשקט → הועלה ל-Adi, שאישרה פתיחה מחדש (2026-08-04).

**תנאי שנשאר:** מסלול ה-join ב-`www.buffadhd.com/join/:code` מאומת E2E לפני ש-copy של הכרטיס מבטיח לינק; עד אז הכרטיס מציג קוד משפחה בלבד (ראה `child-access-paths` SPEC §DG2/A2).

**מסמכים מושפעים:**
- `docs/sessions/child-access-paths/SPEC.md` (v3, §DG5) + REVIEW.md
- מתקן: `docs/sessions/onboarding-redesign/SPEC.md` §DG1 (החלטה קודמת — לא נמחקת)
...
**ההחלטה:** מודל ההכנסה של BUFF להשקה (מעולם לא ננעל קודם — DECISIONS_LOG היה ריק מהנושא):
- **Freemium נדיב** — **חינם:** כל לולאת ההרגל + המעורבות (משימות עד **6**/ילד, BUFFs+חנות+מימוש, **Buddy מלא + skins**, מיני-משחק) + כל רשת-הביטחון הרגשית (Vibe Check, SOS/Low Power, Anchor Recovery, Pause, התראות). **בתשלום:** משימות **7+**, ילד 2+, **תובנות הורה**, **הכנת תיק**, **יציאה משגרה**, **מערכת שעות (+AI)**, **פעילויות**, **לכידת משימות מטקסט/תמונה** (עתידי). עיקרון: חינם = ליבה+רגש (שההרגל ייווצר, לא לחסום ילד במצוקה); תשלום = סקייל + אקסטרות + מה שעולה לנו כסף (AI). גבול המשימות (לא מספר הילדים) הוא המנוף הראשי. (גבול 6 נאכף בקוד; קודם היה מיוצא אך לא נאכף.)
- **תמחור:** חודשי **$9.99**, שנתי **$59.99** (דגש על שנתי), **ללא תקופת ניסיון** (שכבת החינם = "נסה לפני שתקנה"; טקסט "14-day free trial" בקוד יתוקן ל-free framing).
- **Founding 100** נשאר כגיבור-השקה חד-פעמי מוגבל ($99 ל-50 הראשונים / $149 לבאים).

**הסיבה:**
- הגייט-לפי-ילדים הוא בדיוק "מצב הכשל" של freemium (RevenueCat playbook): רוב המשפחות עם ילד אחד לעולם לא פוגשות גייט → אין מסלול שדרוג → אפס הכנסה. זה היה החשש של Adi, ואומת.
- deep-research (2026-06-19, 25 מקורות, אימות אדוורסרי): Finch ($30M ARR, מקבילה קרובה) = אותו מודל בדיוק (ליבה חינם, פרימיום על תוספות). hard-paywall היה חותך את המשתמשים של BUFF לפני שההרגל נוצר (ערך לוקח שבועות, 79% נטשו). מנוי שנתי שומר פי-2.6.
- נדחו: hard-paywall (מסוכן ל-habit שביר), המשך גייט-לפי-ילדים (לא ממכר לרוב), תקופת ניסיון קצרה (55% מבטלים ביום-0).

**פתוח:** WTP אמיתי של משפחת ילד-יחיד (ההימור הנותר); Values Check על גבול 6 המשימות; ואיכות התובנות מספקת להצדקת תשלום (תזכורת הוקמה).

**מסמכים מושפעים:**
- `docs/sessions/monetization-model/SPEC.md` — SPEC מלא (this commit)
- קוד: `useSubscription.ts` (אכיפת FREE_TASK_LIMIT), `useChildProgress.addTask`, `en.json`/`he.json` (טקסט ניסיון) — חבילה נפרדת, ממתינה לביצוע
- Play Console / RevenueCat — מחירים + מוצרים (הגדרה ידנית)
