...
### IN-2026-09-02-01: tomorrow-pack-inconsistency P1 — הבדיקה חשפה מרוץ בין tap ראשון לקריאת AsyncStorage; PackingCard הפך למשטח האריזה היחיד
- **תאריך:** 2026-09-02
...
- **השפעה:** P1 שונה מ"הבלטה ויזואלית" ל: פיל+פס להיום, פיל עמום עם יום-בשבוע ורמז למחר, שער loading בתוך הכרטיס (מתקן גם הבהוב `camp.empty` קיים בדשבורדים), `useFocusEffect` לסנכרון סימונים, וסגירת "מוכנים!" קריאה ב-Mint. 18 בדיקות jest חדשות. אימות: tsc 0, jest 1009/1009, web export נקי. **אומת על אנדרואיד (Hat-3, 2026-09-05):** טאב ציוד מציג את החוגים למחר בלי "יום חופש", מפקדה מקופלת→נפתחת, סנכרון סימונים בין המארחים ללא הפעלה מחדש, שמירה אחרי kill+relaunch, Mint+Gamer, RTL (פס בצד ימין).
- **סטטוס:** `resolved` בקוד (P1–P3 באותו ענף, 2026-09-02) + Hat-3 אנדרואיד עבר (2026-09-05); Hat-4 (אימות ויזואלי סופי של Adi) לפני flag-on.
...
- **A strict "recurring ⇒ weekdays NOT NULL" CHECK would have rejected old-build inserts** during the compat window (they insert `weekday` only). Shipped a **weak** CHECK (`weekdays IS NULL OR jsonb_array_length > 0`) — forbids `[]`, doesn't force presence. Strict CHECK deferred.
- **`scheduleColumns` one-off branch had to explicitly null `weekdays`** (SPEC only showed the recurring branch) — else converting recurring→one-off leaves a stale array, re-introducing the exact drift the package exists to prevent.
- **SPEC file-list gap I found while verifying:** `ChildAddActivityScreen.tsx` *also* constructs a recurring schedule and would break the type change — not in the SPEC. Handled as a minimal wrap (child stays single-select, emits `weekdays:[weekday]`).
- **PM cut the ceremony:** the SPEC's dedicated "cleanup package" for dropping `weekday` was over-built for 9 rows; folded the eventual drop into any future activities migration. Kept the one-line dual-write only.
- **UX: the multi-select gesture already ships twice** (`DayScheduleToggles`, `MedReminderSheet`) — added a persistent hint line (solves discoverability *and* the silent min-1 guard) + a11y selected-state to match.
- **Gotcha (process):** the branch was cut off `origin/main` which already had #422 (added a third `.eq('is_deleted', false)` to the Activities children query). A new jest suite copied a *stale* 2-`.eq()` supabase mock from earlier context and failed with `.eq is not a function`. Lesson: when mocking a chained query, read the CURRENT screen's query on the actual branch, not a remembered shape. Migration 054 applied to prod + backfill verified (6 rows, 0 mismatch).
...
- **תאריך:** 2026-07-29
- **מקור:** CC — תוך כדי איסוף נתונים לבדיקת קופי מול Gemini, בהקשר של `sessions/smart-organizer-discovery`
- **תיאור:** ה-red team של 28/7 (ממצא F6) קבע שאישור לכידה "לא מייצר שום דבר נראה", והסיק מכך שיעור חזרה 0%. האבחנה הטכנית הייתה **נכונה לגמרי** — משימה עם תאריך מופיעה אצל הילד רק ביום שלה, וההרצה האמיתית הפיקה פריטים לתאריכים 26/7–6/8 — אבל המסגור היה שגוי. שאילתה פשוטה על `capture_runs` בפילוח לפי `family_id` מראה: **9 הרצות אי פעם, 8 מהמשפחה של Adi (`37d6a2bd…`) ואחת מחשבון בדיקה `ParentTest520`.** כל 19 ה-`parent_items` בפרודקשן שייכים למשפחה אחת — של Adi. גם "ההרצה האמיתית של 24/7" שעליה נשענת האבחנה היא הרצה של Adi. כלומר לא היה מעולם קהל שיכול היה לחזור. **המספר היה בכל הדוחות; מה שחסר היה `group by family_id`.**
- **השפעה:** (1) חבילה שלמה (#406, קבלת הלכידה) נבנתה כדי לתקן חזרה — היא עדיין נכונה כתיקון, אבל היא לא יכלה להזיז את המדד שהיא נמדדת מולו, כי המדד היה 0 מסיבה אחרת. (2) **מלכודת מבנית:** מדד שנמדד על "כל הפרודקשן" בלי להחריג את משפחת המפתחת יראה תמיד תנועה, וייראה כמו מוצר חי. `scripts/capture-discovery.sql` נכתב עם רשימת החרגה מפורשת בכל בלוק בדיוק בגלל זה. (3) התקרה שהתגלתה באותה בדיקה — 23 הורים שנראו ב-30 יום, **4 ב-7 יום**, מתוך 234 משפחות — היא ההקשר שחסר לכל דיון "איך משפרים פיצ'ר X". (4) הפייוול הוסר כחשוד: מאז #404 יש 3 הרצות חינם לכל משפחה, ו-`error_premium` מעולם לא נרשם.
- **סטטוס:** `resolved` לגבי המסגור — `pkg/capture-discovery` (P1+P2) מטפל בגילוי ומוסיף `capture_entry_seen`/`capture_entry_tapped` כדי שהמשפך יהיה נראה. **נשאר פתוח:** P3 (שיתוף ישיר מוואטסאפ) טעון אישור תלות + build; ושאלת ההפצה — האם משטח בתוך אפליקציה עם 4 הורים שבועיים הוא בכלל המקום להשקיע בו.
- **קשור ל:** `sessions/smart-organizer-discovery/SPEC.md` · `docs/research/RED_TEAM_AI_FEATURES_2026-07-28.md` (F6) · #406 · #409 · `project_capture_beta_live`

### IN-2026-07-27-01: קופי מתורגם ≠ קופי שמישהו רואה — 4 מפתחות `philosophy.community.*` ישבו בשני הקטלוגים חודשים בלי שאף קומפוננטה קראה להם, ולכן האפליקציה מעולם לא הזמינה אף אחד לקהילת הוואטסאפ

- **תאריך:** 2026-07-27
- **מקור:** CC — בעקבות שאלה של Adi "למה אין מצטרפים חדשים לקבוצת הוואטסאפ?", בהקשר של `sessions/community-surfacing`
- **תיאור:** שתי קבוצות הוואטסאפ (HE + EN) חיות ותקינות (שני הקישורים נבדקו 2026-07-27 ומחזירים דף הצטרפות תקף). הקישור מופיע ב-`landing-web` פעמיים — פוטר (`Landing.tsx:402`) ודף `/about` (`AboutPage.tsx:24`). **באפליקציה עצמה — אפס משטחים.** המפתחות `philosophy.community.title/insight/buttonText/link` קיימים בשני הקטלוגים (`he.json` / `en.json` שורות 998–1001), מתורגמים, **וכל אחד מצביע על הקבוצה הנכונה לשפה שלו** — אבל `grep` על כל `src/` מראה שאף קומפוננטה לא קוראת להם. הם הגיעו בפורט מ-Lovable; `PhilosophyScreen.tsx` מרנדר עמודי תווך, עקרונות וציטוט — ונעצר בדיוק לפני הבלוק הזה. כל המשתמשים החדשים נכנסים דרך האפליקציה, וגם דפי המדריכים (`/guides/*`, תעבורת ה-SEO) לא הזכירו את הקהילה בכלל.
- **השפעה:** (1) הסבר מלא לקיפאון בגיוס לקהילה — לא בעיית תוכן אלא בעיית נראות. (2) **מלכודת מבנית כללית:** בדיקות ה-i18n שלנו מוודאות *התאמה בין קטלוגים* (2197=2197 מפתחות, אפס יתומים לשני הכיוונים) — שזה בדיוק מה שהחמיץ את זה: מפתח שקיים בשתי השפות ואף אחד לא קורא לו עובר את כל הבדיקות. אין שום בדיקה ש"כל מפתח בקטלוג מגיע ממקום כלשהו ב-`src/`". (3) התיקון עצמו היה זול — לרנדר קופי שכבר נכתב, תורגם ואושר. (4) הערך של המפתח הוא ה-URL עצמו, ולכן בחירת הקבוצה לפי שפה מתקבלת חינם מ-i18next — בלי אף `language === 'he' ?` בקוד, שזה בדיוק הדפוס שדלף בעבר locale של מכשיר למסכי ילד.
- **סטטוס:** `resolved` — `pkg/community-surfacing` מרנדר את הבלוק ב-4 משטחים (Settings, Philosophy, סוף אונבורדינג, ו-8 בלוקים בדפי המדריכים). **נשאר פתוח כרעיון:** בדיקת "מפתחות יתומים" (מפתח בקטלוג שאין לו קורא ב-`src/`) — לא בסקופ של החבילה הזו.
- **קשור ל:** `sessions/community-surfacing/SPEC.md` · `project_i18n_three_language_sources` · IN-2026-07-08-01 (אותה קהילה, צד ה-landing)
