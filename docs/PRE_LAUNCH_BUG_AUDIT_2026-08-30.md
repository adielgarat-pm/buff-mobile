...
- **✅ FIXED 2026-08-31** — shared local day helper `src/lib/dayKey.ts`; all "child's today" keys (`useChildProgress`, `useChildrenDashboard`, `vibeUtils`→vibe check, `usePetState` streak, `useAnchorRecoveryDismiss`) now roll at local midnight. This fix also resolves **H5**. Regression: 950/950 tests green + new `dayKey.test.ts`. SPEC + device-verification steps in `docs/PRE_LAUNCH_HIGH_FIX_SPECS.md`. *Device confirmation (emulator TZ = US Pacific) still owed.*
