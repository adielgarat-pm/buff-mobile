...
12. **נתיב נייטיבי חדש = ראיה מאנדרואיד, לא "verified".** פיצ'ר שנוגע ב-API נייטיבי (קבצים, מצלמה, התראות, אחסון, שיתוף) לא מקבל feature-flag ON ולא שורת RELEASE_QUEUE בלי **ראיה מהרצה על אנדרואיד** (צילום מסך / לוג מ-Hat-3 באמולטור או ממכשיר) של הנתיב הנייטיבי עצמו. typecheck + jest + ווב אינם ראיה: tsc מקבל APIs שזורקים בזמן ריצה (deprecated stubs), jest ממקק נייטיב, ולווב יש נתיבי קוד נפרדים — שלושתם היו ירוקים כשנתיב הקבצים של Capture נשלח מת לאנדרואיד. _(מקור: Lesson 2026-07-22 — `expo-file-system` root import; הבטא הודלקה עם "typecheck + web verified" ונתיב הקובץ מעולם לא הורץ.)_
13. **דחייה = FLAG, לא הערת קוד.** כל החלטה לדחות חלק מההיקף ("נשלים אחר כך" / "wiring deferred if needed") חייבת להירשם כ-🚩 ב-`INTEGRATION_LEARNINGS.md` (ובמידת הצורך שורה ב-GAP_ANALYSIS) **לפני ה-merge**. הערת קוד בלבד בלתי-נראית ללולאה — "נדחה" הופך בשקט ל"נשלח בלי זה". _(אותו IN — ה-deep-link מהתראה נדחה כהערת קוד פעמיים, מעולם לא סומן, ולכן נשלח חסר.)_
