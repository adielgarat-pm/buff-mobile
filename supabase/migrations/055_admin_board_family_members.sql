-- 055_admin_board_family_members.sql
--
-- The tester board only surfaced ONE parent per family (first by created_at)
-- and one family-level platform, so co-parent families (e.g. one parent on
-- Android, one on web) rendered as a single-parent family and the second
-- parent was invisible. Child rows also had no device info, hiding the
-- own-device vs shared-device distinction.
--
-- Adds, without touching any existing field (backward compatible):
--   * family.parents[]  — every non-deleted parent profile with email,
--     per-profile platform / last_seen / last_country, and
--     auth.users.last_sign_in_at (server truth — client telemetry does not
--     stamp last_platform/last_seen_at on web, see Shira 2026-08-03).
--   * child.last_platform / last_seen_at / last_sign_in_at / own_device
--     (own_device = child profile has its own auth user).
